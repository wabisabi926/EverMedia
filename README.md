...
| **FFProbe 最大重试次数** (`MaxProbeRetries`) | 防止项目失效后，emby不停访问导致死循环。 |
| **FFProbe 失败重置时间 (分钟)** (`ProbeFailureResetMinutes`) | 项目获取失败后，在一定时间内，不再重复访问，直接跳过。这个时间过后，如果任务运行，则继续访问。|
| **计划任务 - 线程数量** (`MaxConcurrency`) | 同时执行项目的媒体信息的数量，例如： 2就是同时执行2个项目。 |
| **计划任务 - .strm 访问间隔（秒）** (`BootstrapTaskRateLimitSeconds`)| 访问远程的时间间隔，例如：2就是2个项目之间最少要间隔2秒。0就是没有限制。请根据自己的情况定义，默认偏保守。 |
